...
static void kernel_heat_3d(int tsteps, int n, double A[120][120][120], double B[120][120][120]) {
   ...
   ****************************************/
   for(t = 1; t <= 500; t++) {
      ...
      for(i = 1; i < n - 1; i++) {
         // #pragma omp parallel for default(shared) private(j, k) firstprivate(n, i, A)
         for(j = 1; j < n - 1; j++) {
            // #pragma omp parallel for default(shared) private(k) firstprivate(n, i, j, A)
            for(k = 1; k < n - 1; k++) {
      ...
      for(i = 1; i < n - 1; i++) {
         // #pragma omp parallel for default(shared) private(j, k) firstprivate(n, i, B)
         for(j = 1; j < n - 1; j++) {
            // #pragma omp parallel for default(shared) private(k) firstprivate(n, i, j, B)
            for(k = 1; k < n - 1; k++) {
