...
static void init_array(int ni, int nj, int nk, int nl, int nm, double A[800][1000], double B[1000][900], double C[900][1200], double D[1200][1100]) {
   ...
   for(i = 0; i < ni; i++) {
      ...
      for(j = 0; j < nk; j++) A[i][j] = (double) ((i * j + 1) % ni) / (5 * ni);
   }
   ...
   for(i = 0; i < nk; i++) {
      ...
      for(j = 0; j < nj; j++) B[i][j] = (double) ((i * (j + 1) + 2) % nj) / (5 * nj);
   }
   #pragma omp parallel for default(shared) private(i, j) firstprivate(nj, nm, nl)
   for(i = 0; i < nj; i++) {
      // #pragma omp parallel for default(shared) private(j) firstprivate(nm, i, nl)
      for(j = 0; j < nm; j++) C[i][j] = (double) (i * (j + 3) % nl) / (5 * nl);
   }
   #pragma omp parallel for default(shared) private(i, j) firstprivate(nm, nl, nk)
   for(i = 0; i < nm; i++) {
      // #pragma omp parallel for default(shared) private(j) firstprivate(nl, i, nk)
      for(j = 0; j < nl; j++) D[i][j] = (double) ((i * (j + 2) + 2) % nk) / (5 * nk);
   }
}
...
static void print_array(int ni, int nl, double G[800][1100]) {
   ...
   fprintf#74{fprintf(stderr, "\n")}
   fprintf#76{fprintf(stderr, "%0.2lf ", G[i][j])}
   ****************************************/
   for(i = 0; i < ni; i++) {
      /*************** Clava msgError **************
      Variables Access as passed arguments Can not be traced inside of function calls :
