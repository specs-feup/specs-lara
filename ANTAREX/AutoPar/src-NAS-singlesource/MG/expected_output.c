...
int main()
{
   ...
   ****************************************/
   ...
   printf#259{printf("  iter %3d\n", it)}
   mg3P#261{mg3P(u, v, r, a, c, n1, n2, n3)}
   resid#262{resid(u, v, r, n1, n2, n3, a, k)}
   ****************************************/
   for (it = 1; it <= nit; it++)
   {
...
void mg3P(double u[], double v[], double r[], double a[4], double c[4], int n1, int n2, int n3)
{
   ...
   /*************** Clava msgError **************
   Variables Access as passed arguments Can not be traced inside of function calls :
   rprj3#405{rprj3(&r[ir[k]], m1[k], m2[k], m3[k], &r[ir[k - 1]], m1[k - 1], m2[k - 1], m3[k - 1], k)}
   ****************************************/
   for (k = lt; k >= lb + 1; k--)
   {
      j = k - 1;
   ...
   /*************** Clava msgError **************
   Variables Access as passed arguments Can not be traced inside of function calls :
   interp#420{interp(&u[ir[k - 1]], m1[k - 1], m2[k - 1], m3[k - 1], &u[ir[k]], m1[k], m2[k], m3[k], k)}
   resid#424{resid(&u[ir[k]], &r[ir[k]], &r[ir[k]], m1[k], m2[k], m3[k], a, k)}
   psinv#428{psinv(&r[ir[k]], &u[ir[k]], m1[k], m2[k], m3[k], c, k)}
   ****************************************/
   for (k = lb + 1; k <= lt - 1; k++)
   {
      j = k - 1;
...
void zran3(void * oz, int n1, int n2, int n3, int nx, int ny, int k)
{
   ...
   /*************** Clava msgError **************
   Variables Access as passed arguments Can not be traced inside of function calls :
   vranlc#865{vranlc(d1, &xx, a, &(z[i3][i2][1]))}
   ****************************************/
   for (i3 = 1; i3 < e3; i3++)
   {
      x1 = x0;
   ...
   /*************** Clava msgError **************
   Variables Access as passed arguments Can not be traced inside of function calls :
   bubble#942{bubble(ten, j1, j2, j3, mm, 1)}
   bubble#949{bubble(ten, j1, j2, j3, mm, 0)}
   ****************************************/
   for (i3 = 1; i3 < n3 - 1; i3++)
   {
      /*************** Clava msgError **************
...
void showall(void * oz, int n1, int n2, int n3)
{
   ...
   /*************** Clava msgError **************
   Variables Access as passed arguments Can not be traced inside of function calls :
   printf#1077{printf("%6.3f", z[i3][i2][i1])}
   printf#1079{printf("\n")}
   printf#1081{printf("  - - - - - - - \n")}
   ****************************************/
   for (i3 = 0; i3 < m3; i3++)
...
void bubble(double ten[][2], int j1[][2], int j2[][2], int j3[][2], int m, int ind)
{
   ...
   {
      /*************** Clava msgError **************
      Loop contains Invalid Statement -> ReturnStmt#1134
      ****************************************/
      for (i = 0; i < m - 1; i++)
      {
   ...
   {
      /*************** Clava msgError **************
      Loop contains Invalid Statement -> ReturnStmt#1157
      ****************************************/
      for (i = 0; i < m - 1; i++)
      {
         if (ten[i][ind] < ten[i + 1][ind])
