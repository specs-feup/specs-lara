...
int main() {
   ...
   log_file_1.open("/home/pedro/Documents/repositories/specs-lara/ANTAREX/AutotunerMatrixMult/date/src/lat_output/matrix_mul/results/times.txt", std::ios_base::app);
   ...
   while(std::chrono::steady_clock::now() < stop_time_point) {
      ...
      matrix_mult_tiling(A, B, C, N, M, K, 8, 16);
      std::chrono::high_resolution_clock::time_point clava_timing_end_3 = std::chrono::high_resolution_clock::now();
      auto clava_timing_duration_3 = std::chrono::duration_cast<std::chrono::nanoseconds>(clava_timing_end_3 - clava_timing_start_3).count();
      std::cout << "elapsed:" << clava_timing_duration_3 << std::endl;
      std::chrono::high_resolution_clock::time_point clava_timing_end_2 = std::chrono::high_resolution_clock::now();
      auto clava_timing_duration_2 = std::chrono::duration_cast<std::chrono::nanoseconds>(clava_timing_end_2 - clava_timing_start_2).count();
      log_file_1 << "matrix_mul_0 1 " << clava_timing_duration_2 << "ns" << std::endl;
      std::cout << "#" << number_of_the_iteration++ << " ";
      std::cout << "[" << N << "x" << M << "]";
      std::cout << " X ";
      std::cout << "[" << M << "x" << K << "]" << " ";
      std::cout << "   BS1: " << 8 << " BS2: " << 16 << std::endl;
   }
