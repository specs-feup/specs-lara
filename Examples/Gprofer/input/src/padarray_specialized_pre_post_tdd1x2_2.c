/* Implementation file for padarray_specialized_pre_post_tdd1x2_2 */
...
/**
 */
tensor_d* padarray_specialized_pre_post_tdd1x2_2_tdd1x2_2d_1(tensor_d* A, double pad[2], tensor_d** restrict y)
{
   int A_ndims;
   int pad_numel;
   int y_ndims;
   ...
   for(i = 1; i <= y_ndims; ++i){
      ...
      if(i <= pad_numel){
         size_at_i += (pad[i - 1]) * (double) (1 + 1);
      }
      
      y_size->data[i - 1] = size_at_i;
   }
   ...
   size_multiargs_td_2_of_2(A, &s1, &s2);
   for(i2 = 1; i2 <= s2; ++i2){
      for(i1 = 1; i1 <= s1; ++i1){
         (*y)->data[((int) ((double) i1 + (pad[0]) - 1.0)) + ((int) ((double) i2 + (pad[1]) - 1.0)) * (*y)->shape[0]] = A->data[(i1 - 1) + (i2 - 1) * A->shape[0]];
      }
